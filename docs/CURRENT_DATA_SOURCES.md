...
- Drone observation intake and operator console: vendor-neutral mission, telemetry, and observation records from human-operated coastal-surveillance drones and coastal observers. Phase 25C adds a local console for human-entered observations and provenance labels. Phase 25D-A adds metadata-only analyst review fields. Phase 25D-B documents the media attachment storage design and privacy review for future evidence workflows. It does not add DJI, flight-control, image-hosting, computer-vision dependency, autonomous aircraft behavior, media-fetching capabilities, or storage implementation. See [Drone Observation Ingestion](DRONE_OBSERVATION_INGESTION.md), [Drone Operator Console](DRONE_OPERATOR_CONSOLE.md), [Observation Analyst Review](OBSERVATION_ANALYST_REVIEW.md), and [Media Attachment Storage Design](MEDIA_ATTACHMENT_STORAGE_DESIGN.md).
