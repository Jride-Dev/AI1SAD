...
- Drone observation intake and operator console: vendor-neutral mission, telemetry, and observation records from human-operated coastal-surveillance drones and coastal observers. Phase 25C adds a local console for human-entered observations and provenance labels; it does not add DJI, flight-control, image-hosting, computer-vision dependency, or autonomous aircraft behavior. See [Drone Observation Ingestion](DRONE_OBSERVATION_INGESTION.md) and [Drone Operator Console](DRONE_OPERATOR_CONSOLE.md).
