...
- GSAF local import and delta tracker: manually downloaded Global Shark Attack File `.csv`, `.xlsx`, or `.xls` files can be read into internal AI1SAD staging JSON with source provenance, stable fingerprints, duplicate detection, and baseline delta reports. GSAF remains an upstream source, not the final AI1SAD incident model. Imported rows do not create warnings, alerts, replay facts, public feed entries, drone observations, scoring changes, or public raw-row redistribution. Behavioral candidates are provisional hypotheses and do not default to mistaken identity. See [GSAF Import And Delta Tracking](GSAF_IMPORT_AND_DELTA_TRACKING.md).
- Shark-human incident registry: internal Phase 26B schema/service foundations for reviewed AI1SAD case records linked to upstream source rows, citations, future archival metadata, future Vic Hislop corpus claims, source conflicts, official/public species status, internal species hypotheses, retaliation-risk species-disclosure guardrails, behavioral hypotheses, confidence labels, public-safe summaries, and private analyst notes. The registry now includes a restricted first real-world seed case for the September 14, 2026 Glenfield Beach incident involving Mel Ismail, with source links to ABC News September 15 and September 18 reporting, 7NEWS, the Geraldton Guardian, and news.com.au reporting. Species remains officially unconfirmed, no internal species hypothesis is stored, and the damaged surfboard/wetsuit testing is retained only as source-attributed provenance. The registry does not add public ingestion endpoints, warning/alert creation, replay artifacts, scoring changes, public feed entries, drone observations, provider adapters, public speculative species attribution, or scraping. See [Shark-Human Incident Registry](SHARK_HUMAN_INCIDENT_REGISTRY.md).
- Planned Australian archival source lanes: future local/manual metadata capture for Australian archival newspapers, Trove/National Library of Australia references, state library collections, surf lifesaving records, coroner/inquest references where legally accessible, government fisheries/shark-control reports, court/inquest reporting, maritime accident archives, and regional newspaper archives. These lanes are not yet fully ingested. AI1SAD will capture source metadata and citations before full text, preserve OCR uncertainty and rights status, score source confidence before behavioral conclusions, and avoid bulk article text or copyrighted article redistribution until rights and terms are confirmed. See [Australian Archival News Tracker](AUSTRALIAN_ARCHIVAL_NEWS_TRACKER.md).
- Planned Vic Hislop corpus lane: future local/manual metadata and claim tracking for `Vic Hislop Shark Man`, catalogue records, interviews, profiles, Shark Show-era records, media appearances, shark capture records, and case claims. Hislop sources are historically important but not automatically authoritative; claims require corroboration, conflict tracking, controversy flags, and confidence scoring before any incident-registry or behavioral-hypothesis use. See [Vic Hislop Corpus Archive](VIC_HISLOP_CORPUS_ARCHIVE.md).
- Drone observation intake and operator console: vendor-neutral mission, telemetry, and observation records from human-operated coastal-surveillance drones and coastal observers. Phase 25C adds a local console for human-entered observations and provenance labels. Phase 25D-A adds metadata-only analyst review fields. Phase 25D-C adds a disabled-by-default, local-only, metadata-only attachment prototype for private evidence records. It does not add DJI, flight-control, image-hosting, computer-vision dependency, autonomous aircraft behavior, media-fetching capabilities, cloud storage, public media exposure, or binary upload. See [Drone Observation Ingestion](DRONE_OBSERVATION_INGESTION.md), [Drone Operator Console](DRONE_OPERATOR_CONSOLE.md), [Observation Analyst Review](OBSERVATION_ANALYST_REVIEW.md), [Local Media Attachment Prototype](LOCAL_MEDIA_ATTACHMENT_PROTOTYPE.md), and [Media Attachment Storage Design](MEDIA_ATTACHMENT_STORAGE_DESIGN.md).
- UAV operator feedback intake: research-only field requirements and workflow notes from UAV operators, lifeguards, coastal authorities, researchers, and agency teams. Feedback does not create sightings, warnings, alerts, replay facts, surveillance feed entries, or scoring changes. See [UAV Operator Feedback Intake](UAV_OPERATOR_FEEDBACK_INTAKE.md).
...
- Replay-time biological-event freshness: strict replay evaluation uses the scenario timestamp for carcass/fish-kill freshness checks, while live/default warning calls continue to use current wall-clock time. This is a replay-stability fix, not a new provider or data-source lane.
